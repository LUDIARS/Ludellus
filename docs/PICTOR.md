...
Pictor は本来 1M Spheres ベンチが回るレンダラ ([[project_pictor_rive]] / Pictor README の benchmark)。 UniLand では:
...
UniLand 側からは Pictor を opaque な library として使うが、 Pictor 側に **C ABI の export** を足してもらう必要がある:
...
JS 側はこの C ABI を Emscripten の `cwrap` で叩く。 Mobile / Desktop も同じ ABI を経由する。 [[feedback_pictor_no_upper_dep]] のとおり Pictor 自身は UniLand に依存しない (UniLand が Pictor の thin client になる)。
...
- [[feedback_pictor_no_run]] — UniLand 側からは Pictor を実行確認しない (Pictor チームの責務)
